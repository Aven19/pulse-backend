...
class FbaPaymentsReportWorker:
    """Class method to create Fba Payments Report from amazon API's"""

    @classmethod
    def create_fba_reimbursements_report(cls, data) -> None:
        """This method creates the Fba reimbursements report from amazon"""
        with app.app_context():
            ...
            try:
                ...
                AzReport.add(account_id=account_id, seller_partner_id=asp_id, request_start_time=start_datetime,
                             request_end_time=end_datetime, type=ASpReportType.FBA_REIMBURSEMENTS_REPORT.value, reference_id=response['reportId'])
                ...
                error_message = 'Error while creating FBA Payments Reimbursement Report in FbaPaymentsReportWorker.create_fba_reimbursements_report(): ' + \
                    str(e)
                logger.error(error_message)
                logger.error(traceback.format_exc())
                send_error_notification(email_to=config_data.get('SLACK').get('NOTIFICATION_EMAIL'), subject='FbaPaymentsReportWorker (FBA REIMBURSEMENTS) Create Report Failure',
                                        template='emails/slack_email.html', data={}, error_message=error_message, traceback_info=traceback.format_exc())

    @classmethod
    def get_fba_reimbursements_report(cls, data):
        """This method gets the Fba Payments Reimbursement reports from amazon and inserts into databse"""
        with app.app_context():
            job_id = data.get('job_id')
            ...
            try:
                ...
                if 'url' in get_report:
                    ...
                    logger.error(
                        'file url not found for Fba Payments Reimbursement report')
                    raise Exception
                ...
                if file_data.status_code != 200:
                    logger.error(
                        'Exception occured while retrieving Fba Payments Reimbursement report')
                    raise Exception
                ...
                error_message = 'Error while retrieving report in FbaPaymentsReportWorker.get_fba_reimbursements_report(): ' + \
                    str(e)
                logger.error(error_message)
                logger.error(traceback.format_exc())
                send_error_notification(email_to=config_data.get('SLACK').get('NOTIFICATION_EMAIL'), subject='FbaPaymentsReportWorker (FBA REIMBURSEMENTS) Download Report Failure',
                                        template='emails/slack_email.html', data={}, error_message=error_message, traceback_info=traceback.format_exc())
